...
export const DarkModeToggle: React.FC<DarkModeToggleProps> = ({ isDark, toggle }) => {
  return (
    ...
    >
      ...
      />
      <Moon className={`absolute left-1.5 w-3 h-3 text-gray-600 transition-opacity duration-300 ${isDark ? 'opacity-0' : 'opacity-100'}`} />
      <Sun className={`absolute right-1.5 w-3 h-3 text-yellow-500 transition-opacity duration-300 ${isDark ? 'opacity-100' : 'opacity-0'}`} />
    </button>
  );
};